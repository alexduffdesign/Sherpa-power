...
class SectionChatbotUI extends ChatbotUI {
   ...
   * @private
   * @param {Object} device - Device answer data
  ...
  /**
   * Toggle visibility of device answer cards
   * @private
   */
  toggleDevicesView() {
    const allCards = this.deviceAnswerGrid.querySelectorAll(
  ...
  /**
   * Update devices view based on the number of cards
   * @private
   */
  updateDevicesView() {
    if (!this.deviceAnswerGrid) return;
  ...
  /**
   * Display an error message with section-specific styling
   * @override
