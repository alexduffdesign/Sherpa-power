import{p as c}from"./markdown-util.DadijVe3.js";import{S as h}from"./streaming-markdown-parser.Bd3zPGns.js";function m(r,t,e=5){return new Promise(n=>{let i=0;r.textContent="";let o=performance.now();const a=s=>{s-o>=e&&i<t.length&&(r.textContent+=t[i],i++,o=s),i<t.length?requestAnimationFrame(a):n()};requestAnimationFrame(a)})}async function d(r,t,e=2){const n=document.createElement("div");n.innerHTML=t,r.innerHTML="";for(const i of n.childNodes)if(i.nodeType===Node.TEXT_NODE){if(i.textContent.trim()){const o=document.createElement("span");r.appendChild(o),await m(o,i.textContent,e)}}else if(i.nodeType===Node.ELEMENT_NODE){const o=i.cloneNode(!1);r.appendChild(o),await d(o,i.innerHTML,e)}}class u extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._eventBus=null}set eventBus(t){this._eventBus=t}connectedCallback(){const t=this.getAttribute("label"),e=this.getAttribute("payload");this.render(t,e)}render(t,e){this.shadowRoot.innerHTML=`
      <style>
        .button-container {
          display: flex;
          flex-wrap: wrap;
    ...
    `,this.carouselContainer=this.shadowRoot.querySelector(".carousel__container"),this.leftButton=this.shadowRoot.querySelector(".carousel__button--left"),this.rightButton=this.shadowRoot.querySelector(".carousel__button--right"),this.carouselData.cards.forEach((e,n)=>{const i=document.createElement("div");i.classList.add("carousel__item");const o=document.createElement("div");o.classList.add("carousel__item-wrapper");const a=document.createElement("div");if(a.classList.add("carousel__item-content"),e.imageUrl){const s=document.createElement("img");s.src=e.imageUrl,s.alt=e.title||"",s.classList.add("carousel__item-image"),a.appendChild(s)}if(e.title){const s=document.createElement("h6");s.classList.add("carousel__item-title"),s.textContent=e.title,a.appendChild(s)}if(e.description&&e.description.text){const s=document.createElement("p");s.classList.add("carousel__item-description"),s.textContent=e.description.text,a.appendChild(s)}if(e.buttons&&e.buttons.length>0){const s=e.buttons[0],l=document.createElement("button");l.classList.add("button","carousel__item-button"),l.setAttribute("data-button-index",n),l.setAttribute("data-button-payload",JSON.stringify(s.request)),l.setAttribute("data-button-text",s.name),l.textContent=s.name||"Select",a.appendChild(l),l.addEventListener("click",this.handleButtonClick)}o.appendChild(a),i.appendChild(o),this.carouselContainer.appendChild(i),this.items.push(i)}),this.initCarousel(),this.leftButton.addEventListener("click",this.moveLeft),this.rightButton.addEventListener("click",this.moveRight),window.addEventListener("resize",this.handleResize),this.updateVisibility(),this.updatePosition()}initCarousel(){this.isDesktop=window.matchMedia("(min-width: 1000px)").matches,this.itemsPerSlide=this.isDesktop?2:1,this.currentIndex=0,this.updatePosition(),this.updateVisibility()}updatePosition(){const t=this.isDesktop?50:100,e=-(this.currentIndex*t);this.carouselContainer.style.transform=`translateX(${e}%)`,this.items.forEach(n=>{n.style.display="flex",n.style.flex=`0 0 ${t}%`})}updateVisibility(){const t=this.items.length-(this.isDesktop?2:1);this.currentIndex<=0?this.leftButton.setAttribute("disabled",""):this.leftButton.removeAttribute("disabled"),this.currentIndex>=t?this.rightButton.setAttribute("disabled",""):this.rightButton.removeAttribute("disabled")}moveLeft(){this.currentIndex<=0||(this.currentIndex--,this.updatePosition(),this.updateVisibility())}moveRight(){const t=this.items.length-(this.isDesktop?2:1);this.currentIndex>=t||(this.currentIndex++,this.updatePosition(),this.updateVisibility())}handleResize(){const t=this.isDesktop;this.isDesktop=window.matchMedia("(min-width: 1000px)").matches,t!==this.isDesktop&&(this.currentIndex=0,this.itemsPerSlide=this.isDesktop?2:1,this.updatePosition()),this.updateVisibility()}handleButtonClick(t){if(!this._eventBus){console.error("No eventBus assigned to CarouselComponent");return}const e=t.target,n=parseInt(e.getAttribute("data-button-index"),10),i=this.carouselData.cards[n];if(!i||!i.buttons||i.buttons.length===0){console.warn("No button data found for this card.");return}const o=i.buttons[0];console.log("Original button data:",o);const a=o.request.payload.title,s=a?`Selected ${a}`:"Selected Power Station";this._eventBus.emit("buttonClicked",{action:o.request,label:s}),this.remove()}}customElements.define("button-component",u);customElements.define("message-component",p);customElements.define("carousel-component",g);console.log("MessageComponent defined");console.log("ButtonComponent defined");console.log("CarouselComponent defined");
//# sourceMappingURL=chatbot-components.D-FY4OO-.js.map
