import{S as l,p as c}from"./markdown-util.B3nWrKld.js";class m extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._eventBus=null}set eventBus(e){this._eventBus=e}connectedCallback(){const e=this.getAttribute("label"),t=this.getAttribute("payload");this.render(e,t)}render(e,t){this.shadowRoot.innerHTML=`
    ...
    `,this.shadowRoot.querySelector(".button").addEventListener("click",()=>{if(!this._eventBus){console.error("No eventBus assigned to ButtonComponent");return}try{const n=JSON.parse(t);this._eventBus.emit("buttonClicked",n)}catch(n){console.error("Error parsing button payload:",n)}})}}class h extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.isStreaming=!1,this.animationFrameId=null,this.defaultAnimationSpeed=15,this.currentAnimationSpeed=this.defaultAnimationSpeed,this.streamingParser=null}connectedCallback(){const e=this.getAttribute("sender"),t=this.getAttribute("content")||"";this.isStreaming=this.hasAttribute("streaming");const n=this.hasAttribute("fromhistory"),a=this.getAttribute("data-animate")!=="false";this.render(e),e==="assistant"?this.isStreaming?t&&(this.streamingParser=new l(o=>{this.appendHTMLContent(o)}),this.streamingParser.appendText(t)):!n&&a&&t?this.animateNonStreamedContent(t):this.directAppendContent(t):this.directAppendContent(t)}appendHTMLContent(e){const t=this.shadowRoot.querySelector(".message__content");if(t){const n=document.createElement("div");for(n.innerHTML=e;n.firstChild;)t.appendChild(n.firstChild);this.scrollToBottom()}}finalizeContentAndAnimate(){this.streamingParser&&(this.streamingParser.end(),this.streamingParser=null)}animateNonStreamedContent(e){const t=this.shadowRoot.querySelector(".message__content");if(!t)return;const n=c(e),s=document.createElement("div");s.innerHTML=n,this.animateNodesSequentially(t,s.childNodes)}async animateNodesSequentially(e,t){for(const n of t)if(n.nodeType===Node.TEXT_NODE){if(n.textContent.trim()==="")continue;await this.animateTextNode(e,n.textContent)}else if(n.nodeType===Node.ELEMENT_NODE){const s=n.tagName.toLowerCase(),a=document.createElement(s);Array.from(n.attributes).forEach(o=>{a.setAttribute(o.name,o.value)}),e.appendChild(a),await this.animateNodesSequentially(a,n.childNodes)}}animateTextNode(e,t){return new Promise(n=>{let s=0;const a=document.createElement("span");e.appendChild(a);let o=performance.now();const i=r=>{r-o>=this.currentAnimationSpeed&&(a.textContent+=t[s],s++,this.scrollToBottom(),o=r),s<t.length?this.animationFrameId=requestAnimationFrame(i):n()};this.animationFrameId=requestAnimationFrame(i)})}render(e){const t=e==="assistant";this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          width: 100%;
        ...
        .message {
          ...
          background-color: ${t?"#FFFFFF":"rgba(255, 255, 255, 0.1)"};
          color: ${t?"#231F25":"#FFFFFF"};
          border: ${t?"none":"1px solid #FFFFFF"};
          overflow: hidden;
          font-family: inherit;
          position: relative;
          opacity: 1;
      ...
      <div class="message-wrapper message-wrapper--${e}">
        ${t?'<div class="assistant-icon">🤖</div>':""}
...
//# sourceMappingURL=chatbot-components.Ctf6cydG.js.map
