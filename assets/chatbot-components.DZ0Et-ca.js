import{p as l}from"./markdown-util.DadijVe3.js";import{S as c}from"./streaming-markdown-parser.Bd3zPGns.js";class m extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._eventBus=null}set eventBus(e){this._eventBus=e}connectedCallback(){const e=this.getAttribute("label"),t=this.getAttribute("payload");this.render(e,t)}render(e,t){this.shadowRoot.innerHTML=`
      <style>
        .button-container {
          display: flex;
          flex-wrap: wrap;
    ...
    `,this.shadowRoot.querySelector(".button").addEventListener("click",()=>{if(!this._eventBus){console.error("No eventBus assigned to ButtonComponent");return}try{const i=JSON.parse(t);this._eventBus.emit("buttonClicked",{action:i,label:e})}catch(i){console.error("Error parsing button payload:",i)}})}}class h extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.isStreaming=!1,this.animationFrameId=null,this.defaultAnimationSpeed=5,this.currentAnimationSpeed=this.defaultAnimationSpeed,this.streamingParser=null}connectedCallback(){const e=this.getAttribute("sender"),t=this.getAttribute("content")||"";this.isStreaming=this.hasAttribute("streaming");const i=this.getAttribute("data-animate")!=="false";if(this.render(e,t),!this.isStreaming&&t)if(i)this.animateNonStreamedContent(t);else{const n=l(t),o=this.shadowRoot.querySelector(".message__content");o&&(o.innerHTML=n,this.scrollToBottom())}else this.isStreaming&&t&&(this.streamingParser=new c(n=>{this.appendHTMLContent(n)}),this.streamingParser.appendText(t))}appendContent(e){if(console.log("appendContent:",e),this.streamingParser)this.streamingParser.appendText(e);else{const t=l(e);this.appendHTMLContent(t)}}appendHTMLContent(e){const t=this.shadowRoot.querySelector(".message__content");if(t){const i=document.createElement("div");for(i.innerHTML=e;i.firstChild;)t.appendChild(i.firstChild);this.scrollToBottom()}}finalizeContentAndAnimate(){console.log("finalizeContentAndAnimate called"),this.streamingParser&&(this.streamingParser.end(),this.streamingParser=null)}animateNonStreamedContent(e){const t=this.shadowRoot.querySelector(".message__content");if(!t)return;const i=l(e),n=document.createElement("div");n.innerHTML=i,this.animateNodesSequentially(t,n.childNodes)}async animateNodesSequentially(e,t){const i=new Set(["p","div","h1","h2","h3","ul","ol","li","blockquote","pre","h4","h5","h6"]);for(const n of t)if(n.nodeType===Node.TEXT_NODE){if(n.textContent.trim()==="")continue;await this.animateTextNode(e,n.textContent)}else if(n.nodeType===Node.ELEMENT_NODE){const o=n.tagName.toLowerCase();i.has(o);const a=document.createElement(o);Array.from(n.attributes).forEach(s=>{a.setAttribute(s.name,s.value)}),e.appendChild(a),await this.animateNodesSequentially(a,n.childNodes)}}animateTextNode(e,t){return new Promise(i=>{let n=0;const o=document.createElement("span");e.appendChild(o);let a=performance.now();const s=r=>{r-a>=this.currentAnimationSpeed&&(o.textContent+=t[n],n++,this.scrollToBottom(),a=r),n<t.length?this.animationFrameId=requestAnimationFrame(s):i()};this.animationFrameId=requestAnimationFrame(s)})}render(e,t){const i=e==="assistant";this.shadowRoot.innerHTML=`
      <style>
        h1, h2, h3, h4, h5, h6 {
