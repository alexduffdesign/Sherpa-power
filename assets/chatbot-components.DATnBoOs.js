import{p as c}from"./markdown-util.DadijVe3.js";import{S as h}from"./streaming-markdown-parser.Bd3zPGns.js";function m(r,t,e=5){return new Promise(n=>{let i=0;r.textContent="";let o=performance.now();const a=s=>{s-o>=e&&i<t.length&&(r.textContent+=t[i],i++,o=s),i<t.length?requestAnimationFrame(a):n()};requestAnimationFrame(a)})}async function d(r,t,e=2){const n=document.createElement("div");n.innerHTML=t,r.innerHTML="";for(const i of n.childNodes)if(i.nodeType===Node.TEXT_NODE){const o=document.createElement("span");r.appendChild(o),await m(o,i.textContent,e)}else if(i.nodeType===Node.ELEMENT_NODE){const o=i.cloneNode(!1);r.appendChild(o),await d(o,i.innerHTML,e)}}class u extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._eventBus=null}set eventBus(t){this._eventBus=t}connectedCallback(){const t=this.getAttribute("label"),e=this.getAttribute("payload");this.render(t,e)}render(t,e){this.shadowRoot.innerHTML=`
      <style>
        .button-container {
          display: flex;
          flex-wrap: wrap;
...
//# sourceMappingURL=chatbot-components.DATnBoOs.js.map
