import{p as d}from"./markdown-util.DadijVe3.js";import{S as c}from"./streaming-markdown-parser.Bd3zPGns.js";function m(a,t,e=2){return new Promise(s=>{let n=0;a.textContent="";let o=performance.now();const i=r=>{r-o>=e&&n<t.length&&(a.textContent+=t[n],n++,o=r),n<t.length?requestAnimationFrame(i):s()};requestAnimationFrame(i)})}async function l(a,t,e=2){const s=document.createElement("div");s.innerHTML=t,a.innerHTML="";for(const n of s.childNodes)if(n.nodeType===Node.TEXT_NODE){if(n.textContent.trim()){const o=document.createElement("span");a.appendChild(o),await m(o,n.textContent,e)}}else if(n.nodeType===Node.ELEMENT_NODE){const o=n.cloneNode(!1);a.appendChild(o),await l(o,n.innerHTML,e)}}class h extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._eventBus=null}set eventBus(t){this._eventBus=t}connectedCallback(){const t=this.getAttribute("label"),e=this.getAttribute("payload");this.render(t,e)}render(t,e){this.shadowRoot.innerHTML=`
      <style>
        .button {
    ...
    `,this.shadowRoot.querySelector(".button").addEventListener("click",()=>{if(!this._eventBus){console.error("No eventBus assigned to ButtonComponent");return}try{const s=JSON.parse(e);this._eventBus.emit("buttonClicked",{action:s,label:t})}catch(s){console.error("Error parsing button payload:",s)}})}}class u extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.isStreaming=!1,this.animationFrameId=null,this.defaultAnimationSpeed=5,this.currentAnimationSpeed=this.defaultAnimationSpeed,this.streamingParser=null}connectedCallback(){const t=this.getAttribute("sender"),e=this.getAttribute("content")||"";console.log("[MessageComponent] Connected:",{sender:t,content:e,isStreaming:this.isStreaming,id:this.id||"no-id"}),this.isStreaming=this.hasAttribute("streaming");const s=this.getAttribute("data-animate")!=="false";this.currentAnimationSpeed=parseInt(this.getAttribute("data-animation-speed")||this.defaultAnimationSpeed,5),this.render(t,e);const n=this.shadowRoot.querySelector(".message__content");if(n)if(n.innerHTML="",!this.isStreaming&&e){const o=/<[a-z][\s\S]*>/i.test(e);if(console.log("[MessageComponent] Processing content:",{isHTML:o,content:e,animate:s}),o){const i=e.replace(/>\s+</g,"><").replace(/\n\s*/g,"").trim();s?l(n,i,this.currentAnimationSpeed):n.innerHTML=i}else this.streamingParser=new c(i=>{this.tempContent||(this.tempContent=""),this.tempContent+=i}),this.streamingParser.appendText(e),this.streamingParser.end(),s?l(n,this.tempContent,this.currentAnimationSpeed):n.innerHTML=this.tempContent;this.scrollToBottom()}else this.isStreaming&&(this.streamingParser=new c(o=>{const i=o.replace(/>\s+</g,"><").replace(/\n\s*/g,"").trim();this.appendHTMLContent(i)}),this.streamingParser.appendText(e))}appendContent(t){if(console.log("appendContent:",t),this.streamingParser)this.streamingParser.appendText(t);else{const e=d(t);this.appendHTMLContent(e)}}appendHTMLContent(t){var s,n;console.log("[MessageComponent] Appending HTML:",{htmlSegment:t,currentContent:(n=(s=this.shadowRoot)==null?void 0:s.querySelector(".message__content"))==null?void 0:n.innerHTML});const e=this.shadowRoot.querySelector(".message__content");if(e){const o=document.createElement("div");o.innerHTML=t;const i=r=>{r.nodeType===3&&(r.textContent=r.textContent.replace(/\s+/g," ").trim()),r.childNodes.forEach(i)};for(i(o);o.firstChild;)e.appendChild(o.firstChild);this.scrollToBottom()}}finalizeContentAndAnimate(){console.log("finalizeContentAndAnimate called"),this.streamingParser&&(this.streamingParser.end(),this.streamingParser=null)}render(t,e){const s=t==="assistant";this.shadowRoot.innerHTML=`
      <style>
        h1, h2, h3, h4, h5, h6 {
