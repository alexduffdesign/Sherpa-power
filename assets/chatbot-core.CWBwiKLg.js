...
`),g=h.find(p=>p.startsWith("event:")),v=h.find(p=>p.startsWith("data:")),b=g?g.split(":")[1].trim():"trace",C=v?JSON.parse(u.substring(u.indexOf("data:")+5).trim()):null;b==="trace"?this.processTrace(C):b==="end"&&i.emit(`${this.eventPrefix}:end`,{})}catch(h){console.error("Error parsing SSE event:",h)}})}}catch(s){i.emit(`${this.eventPrefix}:typing`,{isTyping:!1}),s.name==="AbortError"?console.warn("SSE connection aborted"):(console.error("SSE connection error:",s),i.emit(`${this.eventPrefix}:error`,{message:s.message}))}finally{this.abortController=null,i.emit(`${this.eventPrefix}:end`,{}),i.emit(`${this.eventPrefix}:typing`,{isTyping:!1})}}processTrace(t){if(!t.type){console.warn("Trace without type received:",t);return}switch(i.emit(`${this.eventPrefix}:typing`,{isTyping:!1}),t.type){case"text":console.log("Text received trace:",t),i.emit(`${this.eventPrefix}:messageReceived`,{content:t.payload.message,metadata:t.payload.metadata||null});break;case"choice":console.log("Choice received trace:",t),i.emit(`${this.eventPrefix}:choicePresented`,{type:"choice",buttons:t.payload.buttons});break;case"carousel":console.log("Carousel received trace:",t),i.emit(`${this.eventPrefix}:carouselPresented`,{type:"carousel",carouselItems:t.payload.cards});break;case"speak":console.log("Speak received trace:",t);break;case"visual":console.log("Visual received trace:",t);break;case"no-reply":console.log("No-reply received trace:",t);break;case"end":console.log("End trace received:",t),i.emit(`${this.eventPrefix}:end`,{});break;case"completion-events":console.log("Completion event trace received:",t);break;default:console.warn(`Unhandled trace type: ${t.type}`,t)}}closeConnection(){this.abortController&&(this.abortController.abort(),console.log("SSE connection closed."))}destroy(){this.closeConnection(),i.removeAllListeners(`${this.eventPrefix}:messageReceived`),i.removeAllListeners(`${this.eventPrefix}:choicePresented`),i.removeAllListeners(`${this.eventPrefix}:carouselPresented`),i.removeAllListeners(`${this.eventPrefix}:deviceAnswer`),i.removeAllListeners(`${this.eventPrefix}:error`),i.removeAllListeners(`${this.eventPrefix}:typing`),i.removeAllListeners(`${this.eventPrefix}:end`)}}class w{constructor(t){if(this.container=t,this.form=this.container.querySelector(".chat-form"),this.input=this.container.querySelector("input[type='text']"),this.messageContainer=this.container.querySelector(".message-container"),this.typingIndicator=this.container.querySelector(".chat-typing"),console.log("Chatbot UI Container:",this.container),console.log("Chat Form:",this.form),console.log("Chat Input:",this.input),console.log("Message Container:",this.messageContainer),console.log("Typing Indicator:",this.typingIndicator),!this.container){console.error("Main Chatbot UI container not found");return}if(!this.form||!this.input){console.error("Main Chatbot form or input not found");return}this.setupEventListeners()}setupEventListeners(){var t;this.form.addEventListener("submit",e=>{e.preventDefault();const s=this.input.value.trim();s&&(i.emit("userMessage",s),this.input.value="")}),i.on(`${l.MAIN_CHATBOT.PREFIX}:typing`,e=>{e.isTyping?this.showTypingIndicator():this.hideTypingIndicator()}),this.messageContainer.addEventListener("click",e=>{const s=e.target.closest("button-component");if(s){const o=JSON.parse(s.getAttribute("payload")||"{}"),n=s.getAttribute("label");n&&(i.emit("buttonClicked",{type:o.type,payload:o,label:n}),this.removeInteractiveElements())}}),(t=document.querySelector("custom-drawer"))==null||t.addEventListener("click",e=>{e.target.closest(".main-menu")&&(e.preventDefault(),i.emit("buttonClicked",{type:"event",payload:{event:{name:"main_menu"}},label:"Back to Menu"}))})}addMessage(t,e,s=null,o=!1){if(!this.messageContainer){console.error("Message container not set");return}!o&&(!s||!s.isInteractive)&&this.removeInteractiveElements();const n=document.createElement("message-component");if(n.setAttribute("sender",t),n.setAttribute("content",e),s!=null&&s.imageUrl&&n.setAttribute("image-url",s.imageUrl),this.messageContainer.appendChild(n),console.log("Message appended to messageContainer"),this.scrollToBottom(),t==="assistant"&&s)switch(s.type){case"choice":this.addButtons(s.buttons,o);break;case"carousel":this.addCarousel(s.carouselItems,o);break}}addButtons(t,e=!1){if(console.log("addButtons called with:",t),!Array.isArray(t)){console.error("addButtons expected an array but received:",t);return}e||(this.removeInteractiveElements(),this.storeInteractiveState("choice",t)),t.forEach(s=>{const o=document.createElement("button-component");o.setAttribute("label",s.name),o.setAttribute("payload",JSON.stringify(s.request)),this.messageContainer.appendChild(o),console.log("Button appended to messageContainer")}),this.scrollToBottom()}addCarousel(t,e=!1){if(console.log("Adding carousel with items:",t),!Array.isArray(t)){console.error("addCarousel expected an array but received:",t);return}e||(this.removeInteractiveElements(),this.storeInteractiveState("carousel",t));const s={cards:t},o=document.createElement("carousel-component");o.setAttribute("data-carousel",JSON.stringify(s)),this.messageContainer.appendChild(o),this.scrollToBottom()}storeInteractiveState(t,e){const s={type:t,data:e,timestamp:Date.now()};localStorage.setItem("lastInteractiveElement",JSON.stringify(s))}restoreInteractiveElement(){const t=localStorage.getItem("lastInteractiveElement");if(t)try{const e=JSON.parse(t);e.type==="choice"?this.addButtons(e.data,!0):e.type==="carousel"&&this.addCarousel(e.data,!0)}catch(e){console.error("Error restoring interactive element:",e)}}showTypingIndicator(){this.typingIndicator&&(this.typingIndicator.style.display="flex",this.scrollToBottom())}hideTypingIndicator(){this.typingIndicator&&(this.typingIndicator.style.display="none")}displayError(t){const e=document.createElement("div");e.classList.add("error-message"),e.innerText=t,this.messageContainer.appendChild(e),this.scrollToBottom()}scrollToBottom(){this.messageContainer.scrollTop=this.messageContainer.scrollHeight}removeInteractiveElements(){this.messageContainer.querySelectorAll("button-component, carousel-component").forEach(e=>e.remove()),localStorage.removeItem("lastInteractiveElement")}}function f(a){const t=Date.now().toString(36)+Math.random().toString(36).substr(2,9);return`${a}-${t}`}class x{constructor(t,e){this.core=t,this.ui=e,this.historyKey="mainChatbotHistory",this.launchKey="chatHasLaunched",this.isLaunched=this.hasLaunched(),this.setupEventListeners()}hasLaunched(){return localStorage.getItem(this.launchKey)==="true"}setLaunched(){localStorage.setItem(this.launchKey,"true"),this.isLaunched=!0}setupEventListeners(){i.on(l.MAIN_CHATBOT.MESSAGE_RECEIVED,t=>{this.ui.addMessage("assistant",t.content,t.metadata),this.saveToHistory("assistant",t.content,t.metadata)}),i.on("userMessage",t=>{this.ui.addMessage("user",t),this.saveToHistory("user",t),this.core.sendMessage(t)}),i.on("buttonClicked",t=>{if(!t||!t.label){console.error("Invalid button data:",t);return}this.ui.addMessage("user",t.label),this.saveToHistory("user",t.label);const e={action:{type:t.type}};t.payload&&Object.keys(t.payload).length>0&&(e.action.payload=t.payload),this.core.sendAction(e)}),i.on(l.MAIN_CHATBOT.CHOICE_PRESENTED,t=>{this.ui.addButtons(t.buttons),this.saveToHistory("assistant","",{type:"choice",buttons:t.buttons})}),i.on(l.MAIN_CHATBOT.CAROUSEL_PRESENTED,t=>{this.ui.addCarousel(t.carouselItems),this.saveToHistory("assistant","",{type:"carousel",carouselItems:t.carouselItems})}),i.on(l.MAIN_CHATBOT.ERROR,t=>{this.ui.displayError(t.message)}),i.on(l.MAIN_CHATBOT.TYPING,t=>{t.isTyping?this.ui.showTypingIndicator():this.ui.hideTypingIndicator()}),document.addEventListener("chatbotLaunch",()=>{this.launch()}),i.on("carouselButtonClicked",t=>{if(!t||!t.action){console.error("Invalid carousel button data:",t);return}this.ui.addMessage("user",t.label),this.saveToHistory("user",t.label),this.core.sendAction({action:t.action})})}launch(){if(this.isLaunched){console.log("Chat already launched, skipping launch request");return}this.core.sendLaunch().then(()=>{console.log("Chatbot launched successfully."),this.setLaunched()}).catch(t=>{console.error("Error launching chatbot:",t),this.ui.displayError("Failed to launch the chatbot. Please try again later.")})}sendMessage(t){const e=this.sanitizeInput(t);this.ui.addMessage("user",e,{type:"message"}),this.saveToHistory("user",e,{type:"message"}),this.core.sendMessage(e).catch(s=>{console.error("Error sending message:",s),this.ui.displayError("Failed to send your message. Please try again.")})}sendAction(t){if(!t||typeof t!="object"){console.error("Invalid action payload:",t),this.ui.displayError("Invalid action triggered.");return}this.core.sendAction({action:t,config:{}}).then(()=>{console.log("Action sent successfully:",t),this.ui.addMessage("user",t.label||"Action executed.",{type:"action"}),this.saveToHistory("user",JSON.stringify(t),{type:"action"})}).catch(e=>{console.error("Error sending action:",e),this.ui.displayError("An error occurred while processing your request.")})}saveToHistory(t,e,s=null){const o=JSON.parse(localStorage.getItem(this.historyKey))||[],n={sender:t,message:e,timestamp:Date.now(),isInteractive:!1};t==="assistant"&&s&&(s.type==="choice"?(n.isInteractive=!0,n.traceType="choice",n.traceData={buttons:s.buttons}):s.type==="carousel"&&(n.isInteractive=!0,n.traceType="carousel",n.traceData={cards:s.carouselItems})),o.push(n),localStorage.setItem(this.historyKey,JSON.stringify(o))}loadHistory(){const t=JSON.parse(localStorage.getItem(this.historyKey))||[];t.forEach((e,s)=>{if(e.isInteractive){s===t.length-1&&(e.traceType==="choice"?this.ui.addButtons(e.traceData.buttons,!0):e.traceType==="carousel"&&this.ui.addCarousel(e.traceData.cards,!0));return}e.message&&this.ui.addMessage(e.sender,e.message)})}restoreInteractiveElement(t){t.traceType==="choice"?this.ui.addButtons(t.traceData.buttons,!0):t.traceType==="carousel"&&this.ui.addCarousel(t.traceData.cards,!0)}sanitizeInput(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}}document.addEventListener("DOMContentLoaded",()=>{const a=document.getElementById("main-chatbot-ui");if(!a){console.error("Main Chatbot UI container not found");return}let t=localStorage.getItem("mainChatbotUserId");t||(t=f("mainChatbot"),localStorage.setItem("mainChatbotUserId",t));const e=new y({userID:t,endpoint:"https://chatbottings--development.gadget.app/voiceflowAPI/voiceflow-streaming",chatbotType:"main"}),s=new w(a);new x(e,s).loadHistory()});class T{constructor(t){if(!t)throw new Error("SectionChatbotUI requires a container element");if(this.container=t,this.form=this.container.querySelector("#chatForm"),this.input=this.container.querySelector("#userInput"),this.messageContainer=this.container.querySelector("#messageContainer"),this.typingIndicator=this.container.querySelector(".chat-typing"),console.log("Chatbot UI Container:",this.container),console.log("Chat Form:",this.form),console.log("Chat Input:",this.input),console.log("Message Container:",this.messageContainer),console.log("Typing Indicator:",this.typingIndicator),!this.form)throw new Error("Chat form not found (id: chatForm)");if(!this.input)throw new Error("Input field not found (id: userInput)");if(!this.messageContainer)throw new Error("Message container not found (id: messageContainer)");this.initializeProductDetails(),this.setupEventListeners()}initializeProductDetails(){this.productDetails={title:this.getAttribute("product-title"),capacity:this.getAttribute("product-capacity"),ac_output_continuous_power:this.getAttribute("product-ac_output_continuous_power"),ac_output_peak_power:this.getAttribute("product-ac_output_peak_power"),dc_output_power:this.getAttribute("product-dc_output_power")},console.log("Initialized product details:",this.productDetails)}setupEventListeners(){this.form.addEventListener("submit",t=>{t.preventDefault();const e=this.input.value.trim();e&&(i.emit("userMessage",e),this.input.value="")}),this.messageContainer.addEventListener("click",t=>{const e=t.target.closest("button-component");if(e){const s=this.getButtonPayload(e);s&&(i.emit("buttonClicked",s),this.removeInteractiveElements())}}),this.messageContainer.addEventListener("click",t=>{if(t.target.closest("carousel-component")){const s=t.target.closest(".carousel-button");if(s){const o=this.getButtonPayload(s);o&&i.emit("carouselButtonClicked",o)}}})}getButtonPayload(t){try{return JSON.parse(t.getAttribute("payload")||"null")}catch(e){return console.error("Error parsing button payload:",e),null}}addMessage(t,e,s=null){const o=document.createElement("message-component");o.setAttribute("sender",t),o.setAttribute("content",e),s&&o.setAttribute("metadata",JSON.stringify(s)),this.messageContainer.appendChild(o),this.scrollToBottom()}addButtons(t){if(!Array.isArray(t)){console.error("Invalid buttons data:",t);return}const e=document.createElement("div");e.className="buttons-container",t.forEach(s=>{const o=document.createElement("button-component");o.setAttribute("label",s.name),o.setAttribute("payload",JSON.stringify(s.request)),e.appendChild(o)}),this.messageContainer.appendChild(e),this.scrollToBottom()}addCarousel(t){if(!Array.isArray(t)){console.error("Invalid carousel items:",t);return}const e=document.createElement("carousel-component");e.setAttribute("items",JSON.stringify(t)),this.messageContainer.appendChild(e),this.scrollToBottom()}showTypingIndicator(){this.typingIndicator&&(this.typingIndicator.style.display="flex",this.scrollToBottom())}hideTypingIndicator(){this.typingIndicator&&(this.typingIndicator.style.display="none")}displayError(t){const e=document.createElement("div");e.classList.add("error-message"),e.innerText=t,this.messageContainer.appendChild(e),this.scrollToBottom()}scrollToBottom(){this.messageContainer.scrollTop=this.messageContainer.scrollHeight}getAttribute(t){return this.container.getAttribute(t)}removeInteractiveElements(){this.messageContainer.querySelectorAll("button-component, carousel-component, .buttons-container").forEach(e=>e.remove())}}class S{constructor(t,e){if(!t||!e)throw new Error("SectionChatbot requires both core and ui instances");this.core=t,this.ui=e,this.isLaunched=!1,this.validateProductDetails(),this.setupEventListeners()}validateProductDetails(){const e=["title","capacity"].filter(s=>!this.ui.productDetails[s]);e.length>0&&(console.error(`Missing required product details: ${e.join(", ")}`),this.ui.displayError("Some product information is missing. Chat functionality may be limited."))}setupEventListeners(){i.on(l.SECTION_CHATBOT.MESSAGE_RECEIVED,t=>{this.ui.hideTypingIndicator(),this.ui.addMessage("assistant",t.content,t.metadata)}),i.on(l.SECTION_CHATBOT.TYPING,t=>{t.isTyping?this.ui.showTypingIndicator():this.ui.hideTypingIndicator()}),i.on(l.SECTION_CHATBOT.CHOICE_PRESENTED,t=>{this.ui.hideTypingIndicator(),this.ui.addButtons(t.buttons)}),i.on(l.SECTION_CHATBOT.CAROUSEL_PRESENTED,t=>{this.ui.hideTypingIndicator(),this.ui.addCarousel(t.carouselItems)}),i.on(l.SECTION_CHATBOT.ERROR,t=>{this.ui.hideTypingIndicator(),this.ui.displayError(t.message)}),i.on("userMessage",t=>{this.sendMessage(t)}),i.on("buttonClicked",t=>{this.handleButtonClick(t)}),i.on("carouselButtonClicked",t=>{this.handleButtonClick(t)}),window.addEventListener("unload",()=>{this.destroy()})}launch(){if(this.isLaunched){console.log("Section chatbot already launched");return}const e={action:{type:"launch",payload:{startBlock:"shopifySection",productDetails:this.sanitizeProductDetails(this.ui.productDetails)}}};console.log("Launching section chatbot with payload:",e),this.ui.showTypingIndicator(),this.core.sendLaunch(e),this.isLaunched=!0}sanitizeProductDetails(t){return Object.entries(t).reduce((e,[s,o])=>(e[s]=o?String(o).trim():"",e),{})}sendMessage(t){t.trim()&&(this.ui.showTypingIndicator(),this.core.sendMessage(t),this.ui.addMessage("user",t))}handleButtonClick(t){t&&(this.ui.showTypingIndicator(),this.core.sendAction({action:t}),this.ui.removeInteractiveElements())}destroy(){this.core.closeConnection(),i.removeAllListeners(`${l.SECTION_CHATBOT.PREFIX}:`),i.removeAllListeners("userMessage"),i.removeAllListeners("buttonClicked"),i.removeAllListeners("carouselButtonClicked")}}document.addEventListener("DOMContentLoaded",()=>{const a=document.getElementById("section-chatbot-ui");if(!a){console.error("Section Chatbot UI container not found");return}let t=localStorage.getItem("sectionChatbotUserId");t||(t=f("sectionChatbot"),localStorage.setItem("sectionChatbotUserId",t));const e=new y({userID:t,endpoint:"https://chatbottings--development.gadget.app/voiceflowAPI/voiceflow-streaming",chatbotType:"section"}),s=new T(a),o=new S(e,s),n=a.querySelector("#userInput");n?n.addEventListener("focus",()=>{o.isLaunched||o.launch()}):console.error("Section Chatbot input field not found")});class A extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){const t=this.getAttribute("label"),e=this.getAttribute("payload");this.render(t,e)}render(t,e){this.shadowRoot.innerHTML=`
    ...
    `,this.shadowRoot.querySelector(".button").addEventListener("click",()=>{try{const s=JSON.parse(e);i.emit("buttonClicked",s)}catch(s){console.error("Error parsing button payload:",s)}})}}class _ extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){const t=this.getAttribute("sender"),e=this.getAttribute("content");this.render(t,e)}render(t,e){const s=t==="assistant";this.shadowRoot.innerHTML=`
      <style>
        message-component {
          width: 100%;
        }
