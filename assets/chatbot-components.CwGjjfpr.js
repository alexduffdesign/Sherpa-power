import{E as y}from"./vendor.DqvJXvYX.js";class C extends y{}const i=new C,E={MAIN_CHATBOT:{MESSAGE_RECEIVED:"mainChatbot:messageReceived",TYPING:"mainChatbot:typing",CHOICE_PRESENTED:"mainChatbot:choicePresented",CAROUSEL_PRESENTED:"mainChatbot:carouselPresented",ERROR:"mainChatbot:error"},SECTION_CHATBOT:{MESSAGE_RECEIVED:"sectionChatbot:messageReceived",DEVICE_ANSWER:"sectionChatbot:deviceAnswer",CHOICE_PRESENTED:"sectionChatbot:choicePresented",CAROUSEL_PRESENTED:"sectionChatbot:carouselPresented",ERROR:"sectionChatbot:error"}};class P{constructor({userID:t,endpoint:e,chatbotType:s}){if(!t)throw new Error("ChatbotCore requires a userID.");if(!e)throw new Error("ChatbotCore requires an endpoint URL.");if(!s)throw new Error('ChatbotCore requires a chatbotType ("main" or "section").');this.userID=t,this.endpoint=e,this.chatbotType=s,this.eventPrefix=s==="main"?"mainChatbot":"sectionChatbot",this.abortController=null,this.initialize()}initialize(){}sendLaunch(t={}){console.log("Constructing launch payload");const e={action:{type:"launch"}};return console.log("Final launch payload:",e),this.sendAction(e)}sendMessage(t){console.log("Constructing message payload:",t);const e={action:{type:"text",payload:t}};return console.log("Final message payload:",e),this.sendAction(e)}async sendAction(t){this.abortController=new AbortController;const{signal:e}=this.abortController;let s=[];try{i.emit(`${this.eventPrefix}:typing`,{isTyping:!0});const o=await fetch(this.endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userID:this.userID,action:t.action}),credentials:"include",signal:e});if(!o.ok){const a=await o.text();throw console.error("API Error Response:",{status:o.status,statusText:o.statusText,body:a}),new Error(`Gadget API responded with status ${o.status}: ${a}`)}const d=o.body.getReader(),b=new TextDecoder("utf-8");let u="";for(;;){const{done:a,value:v}=await d.read();if(a)break;u+=b.decode(v,{stream:!0});const h=u.split(`

`);u=h.pop();for(const c of h)if(c.trim()!=="")try{const l=c.split(`
`),p=l.find(r=>r.startsWith("event:")),f=l.find(r=>r.startsWith("data:")),m=p?p.split(":")[1].trim():"trace",g=f?JSON.parse(c.substring(c.indexOf("data:")+5).trim()):null;if(m==="trace"&&g)s.push(g);else if(m==="end"){for(const r of s)await this.processTrace(r);s=[],i.emit(`${this.eventPrefix}:end`,{}),i.emit(`${this.eventPrefix}:typing`,{isTyping:!1})}}catch(l){console.error("Error parsing SSE event:",l)}}}catch(o){i.emit(`${this.eventPrefix}:typing`,{isTyping:!1}),o.name==="AbortError"?console.warn("SSE connection aborted"):(console.error("SSE connection error:",o),i.emit(`${this.eventPrefix}:error`,{message:o.message}))}finally{this.abortController=null,i.emit(`${this.eventPrefix}:end`,{})}}processTrace(t){if(!t.type){console.warn("Trace without type received:",t);return}switch(t.type){case"text":console.log("Text received trace:",t),i.emit(`${this.eventPrefix}:messageReceived`,{content:t.payload.message,metadata:t.payload.metadata||null});break;case"choice":console.log("Choice received trace:",t),i.emit(`${this.eventPrefix}:choicePresented`,{buttons:t.payload.buttons});break;case"carousel":console.log("Carousel received trace:",t),i.emit(`${this.eventPrefix}:carouselPresented`,{carouselItems:t.payload.cards});break;case"speak":console.log("Speak received trace:",t);break;case"visual":console.log("Visual received trace:",t);break;case"no-reply":console.log("No-reply received trace:",t);break;case"end":console.log("End trace received:",t),i.emit(`${this.eventPrefix}:end`,{});break;case"completion-events":console.log("Completion event trace received:",t);break;default:console.warn(`Unhandled trace type: ${t.type}`,t)}}closeConnection(){this.abortController&&(this.abortController.abort(),console.log("SSE connection closed."))}destroy(){this.closeConnection(),i.removeAllListeners(`${this.eventPrefix}:messageReceived`),i.removeAllListeners(`${this.eventPrefix}:choicePresented`),i.removeAllListeners(`${this.eventPrefix}:carouselPresented`),i.removeAllListeners(`${this.eventPrefix}:deviceAnswer`),i.removeAllListeners(`${this.eventPrefix}:error`),i.removeAllListeners(`${this.eventPrefix}:typing`),i.removeAllListeners(`${this.eventPrefix}:end`)}}class T{constructor(t){if(this.container=t,this.form=this.container.querySelector(".chat-form"),this.input=this.container.querySelector("input[type='text']"),this.messageContainer=this.container.querySelector(".message-container"),this.typingIndicator=this.container.querySelector(".chat-typing"),console.log("Chatbot UI Container:",this.container),console.log("Chat Form:",this.form),console.log("Chat Input:",this.input),console.log("Message Container:",this.messageContainer),console.log("Typing Indicator:",this.typingIndicator),!this.container){console.error("Main Chatbot UI container not found");return}if(!this.form||!this.input){console.error("Main Chatbot form or input not found");return}this.setupEventListeners(),this.setupUIEventListeners()}setupEventListeners(){this.form.addEventListener("submit",t=>{t.preventDefault();const e=this.input.value.trim();e&&(i.emit("userMessage",e),this.input.value="")}),i.on(`${E.PREFIX}:typing`,t=>{t.isTyping?this.showTypingIndicator():this.hideTypingIndicator()})}setupUIEventListeners(){this.container.querySelector(".chatbot-container").addEventListener("click",t=>{const e=t.target.closest("button.button");if(e){const s=JSON.parse(e.getAttribute("data-button-data"));i.emit("buttonClicked",s),this.removeInteractiveElements()}})}onUserMessage(t){i.on("userMessage",t)}onButtonClick(t){i.on("buttonClicked",t)}addMessage(t,e,s=null){if(!this.messageContainer){console.error("Message container not set");return}const o=document.createElement("message-component");if(o.setAttribute("sender",t),o.setAttribute("content",e),this.messageContainer.appendChild(o),console.log("Message appended to messageContainer"),this.scrollToBottom(),this.saveToHistory(t,e,s),t==="assistant"&&s)switch(s.type){case"choice":this.addButtons(s.buttons);break;case"carousel":this.addCarousel(s.carouselItems);break}}addButtons(t){if(console.log("addButtons called with:",t),!Array.isArray(t)){console.error("addButtons expected an array but received:",t);return}t.forEach(e=>{const s=document.createElement("button-component");s.setAttribute("label",e.name),s.setAttribute("payload",JSON.stringify(e.request)),this.messageContainer.appendChild(s),console.log("Button appended to messageContainer")}),this.scrollToBottom()}addCarousel(t){console.log("Adding carousel:",t);const e=document.createElement("carousel-component");e.setAttribute("data-carousel",JSON.stringify(t)),this.messageContainer.appendChild(e),this.scrollToBottom()}showTypingIndicator(){this.typingIndicator&&(this.typingIndicator.style.display="flex",this.scrollToBottom())}hideTypingIndicator(){this.typingIndicator&&(this.typingIndicator.style.display="none")}displayError(t){const e=document.createElement("div");e.classList.add("error-message"),e.innerText=t,this.messageContainer.appendChild(e),this.scrollToBottom()}scrollToBottom(){this.messageContainer.scrollTop=this.messageContainer.scrollHeight}saveToHistory(t,e,s=null){const o=JSON.parse(localStorage.getItem("mainChatbotHistory"))||[];o.push({sender:t,message:e,metadata:s}),localStorage.setItem("mainChatbotHistory",JSON.stringify(o))}removeInteractiveElements(){this.messageContainer.querySelectorAll("button-component, carousel-component").forEach(e=>e.remove())}}class w extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){const t=this.getAttribute("label"),e=this.getAttribute("payload");this.render(t,e)}render(t,e){this.shadowRoot.innerHTML=`
      <style>
        .button-container {
          display: flex;
          flex-wrap: wrap;
    ...
    `,this.shadowRoot.querySelector(".button").addEventListener("click",()=>{try{const s=JSON.parse(e);i.emit("buttonClicked",s)}catch(s){console.error("Error parsing button payload:",s)}})}}class x extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){const t=this.getAttribute("sender"),e=this.getAttribute("content");this.render(t,e)}render(t,e){const s=t==="assistant";this.shadowRoot.innerHTML=`
      <style>
        .message-wrapper {
          display: flex;
