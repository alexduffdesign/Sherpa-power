import{m as d}from"./marked.esm.DtrQ3Nt4.js";function l(c){if(!c)return"";const t=c.replace(/([^\n])(#+ )/g,`$1
$2`);return d.parse(t)}class u extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._eventBus=null}set eventBus(t){this._eventBus=t}connectedCallback(){const t=this.getAttribute("label"),e=this.getAttribute("payload");this.render(t,e)}render(t,e){this.shadowRoot.innerHTML=`
      <style>
        .button-container {
          display: flex;
          flex-wrap: wrap;
    ...
    `,this.shadowRoot.querySelector(".button").addEventListener("click",()=>{if(!this._eventBus){console.error("No eventBus assigned to ButtonComponent");return}try{const s=JSON.parse(e);this._eventBus.emit("buttonClicked",s)}catch(s){console.error("Error parsing button payload:",s)}})}}class h extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.rawChunks=[],this.interimText=""}connectedCallback(){const t=this.getAttribute("sender");this.render(t)}appendContent(t){this.rawChunks.push(t),this.interimText+=this.cleanMarkdown(t),this.updateInterimDisplay()}cleanMarkdown(t){return t.replace(/^(#+)\s/gm,"").replace(/(\*\*|__)(.*?)\1/g,"$2").replace(/(\*|_)(.*?)\1/g,"$2").replace(/^([-+*])\s/gm,"").replace(/`([^`]*)`/g,"$1").trim()}updateInterimDisplay(){const t=this.shadowRoot.querySelector(".message__content");t&&(t.textContent=this.interimText)}finalizeContentAndAnimate(){const t=this.shadowRoot.querySelector(".message__content");if(t){const e=this.rawChunks.join(""),s=l(e);this.finalHTMLContent=s,this.animateBlockTransition(t,s)}}animateBlockTransition(t,e){t.classList.add("fade-out"),t.addEventListener("transitionend",()=>{t.innerHTML=e,t.classList.remove("fade-out"),t.classList.add("fade-in")},{once:!0})}render(t){const e=t==="assistant";this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: block;
          width: 100%;
...
//# sourceMappingURL=chatbot-components.Dpvqg6-f.js.map
