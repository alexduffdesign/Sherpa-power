import{p as d}from"./markdown-util.DadijVe3.js";import{S as c}from"./streaming-markdown-parser.Bd3zPGns.js";function h(a,t,e=2){return new Promise(s=>{let n=0;a.textContent="";let i=performance.now();const o=r=>{r-i>=e&&n<t.length&&(a.textContent+=t[n],n++,i=r),n<t.length?requestAnimationFrame(o):s()};requestAnimationFrame(o)})}async function l(a,t,e=2){const s=document.createElement("div");s.innerHTML=t,a.innerHTML="";for(const n of s.childNodes)if(n.nodeType===Node.TEXT_NODE){if(n.textContent.trim()){const i=document.createElement("span");a.appendChild(i),await h(i,n.textContent,e)}}else if(n.nodeType===Node.ELEMENT_NODE){const i=n.cloneNode(!1);a.appendChild(i),await l(i,n.innerHTML,e)}}class m extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._eventBus=null}set eventBus(t){this._eventBus=t}connectedCallback(){const t=this.getAttribute("label"),e=this.getAttribute("payload");this.render(t,e)}render(t,e){this.shadowRoot.innerHTML=`
      <style>
        .button {
    ...
    `,this.shadowRoot.querySelector(".button").addEventListener("click",()=>{if(!this._eventBus){console.error("No eventBus assigned to ButtonComponent");return}try{const s=JSON.parse(e);this._eventBus.emit("buttonClicked",{action:s,label:t})}catch(s){console.error("Error parsing button payload:",s)}})}}class u extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.isStreaming=!1,this.animationFrameId=null,this.defaultAnimationSpeed=5,this.currentAnimationSpeed=this.defaultAnimationSpeed,this.streamingParser=null,this.contentInitialized=!1}connectedCallback(){if(this.contentInitialized)return;const t=this.getAttribute("sender"),e=this.getAttribute("content")||"";this.isStreaming=this.hasAttribute("streaming");const s=this.getAttribute("data-animate")!=="false";this.currentAnimationSpeed=parseInt(this.getAttribute("data-animation-speed")||this.defaultAnimationSpeed,5),this.render(t,e);const n=this.shadowRoot.querySelector(".message__content");if(n){if(!this.isStreaming&&e){if(/<[a-z][\s\S]*>/i.test(e)){const o=e.replace(/>\s+</g,"><").replace(/\n\s*/g,"").trim();s?l(n,o,this.currentAnimationSpeed):n.innerHTML=o}else this.streamingParser=new c(o=>{this.tempContent||(this.tempContent=""),this.tempContent+=o}),this.streamingParser.appendText(e),this.streamingParser.end(),s?l(n,this.tempContent,this.currentAnimationSpeed):n.innerHTML=this.tempContent;this.scrollToBottom()}else this.isStreaming&&(this.streamingParser=new c(i=>{const o=i.replace(/>\s+</g,"><").replace(/\n\s*/g,"").trim();this.appendHTMLContent(o)}),this.streamingParser.appendText(e));this.contentInitialized=!0}}appendContent(t){if(console.log("appendContent:",t),this.streamingParser)this.streamingParser.appendText(t);else{const e=d(t);this.appendHTMLContent(e)}}appendHTMLContent(t){const e=this.shadowRoot.querySelector(".message__content");if(e){const s=document.createElement("div");s.innerHTML=t;const n=i=>{i.nodeType===3&&(i.textContent=i.textContent.replace(/\s+/g," ").trim()),i.childNodes.forEach(n)};for(n(s);s.firstChild;)e.appendChild(s.firstChild);this.scrollToBottom()}}finalizeContentAndAnimate(){console.log("finalizeContentAndAnimate called"),this.streamingParser&&(this.streamingParser.end(),this.streamingParser=null)}render(t,e){const s=t==="assistant";this.shadowRoot.innerHTML=`
      <style>
        h1, h2, h3, h4, h5, h6 {
